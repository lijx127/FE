...
Slider.prototype = {
  ...
  initialize: function(container, bar, options) {
	this.Bar = hScroll(bar);
	this.Container = hScroll(container);
  ...
  KeyCtrl: function(e) {
	if(this.KeySpeed){
		var iLeft = this.Bar.offsetLeft, iWidth = (document.getElementById("tr").childNodes.length * 70 - this.Bar.offsetWidth) / this.KeySpeed
			, iTop = this.Bar.offsetTop, iHeight = (document.getElementById("tr").childNodes.length * 70 - this.Bar.offsetHeight) / this.KeySpeed;
		//鏍规嵁鎸夐敭璁剧疆鍊?
		switch (e.keyCode) {
			case 37 ://宸?
				iLeft -= iWidth; break;
  ...
  GetPercent: function() {
	//鏍规嵁婊戝姩鏉℃粦鍧楀彇鐧惧垎姣?
	return this._horizontal ? this.Bar.offsetLeft / (document.getElementById("tr").childNodes.length * 70 - this.Bar.offsetWidth)
  ...
  SetPercent: function(value) {
	//鏍规嵁鐧惧垎姣旇缃粦鍧椾綅缃?
	this.EasePos((document.getElementById("tr").childNodes.length * 70 - this.Bar.offsetWidth) * value, (this.Container.clientHeight - this.Bar.offsetHeight) * value);
  },
  //鑷姩婊戠Щ(鏄惁閫掑)
  Run: function(bIncrease) {
	...
	bIncrease = !!bIncrease;
	//鏍规嵁鏄惁閫掑鏉ヨ缃€?
	var percent = this.GetPercent() + (bIncrease ? 1 : -1) * this.RunStep / 500;
	

	this.SetPos((document.getElementById("tr").childNodes.length * 70 - this.Bar.offsetWidth) * percent, (this.Container.clientHeight - this.Bar.offsetHeight) * percent);
...
Drag.prototype = {
  ...
  initialize: function(drag, options) {
	...
	this._Handle = hScroll(this.options.Handle) || this.Drag;
	this._mxContainer = hScroll(this.options.mxContainer) || null;
